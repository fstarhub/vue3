...
hash：地址栏中有#符号，#号不会被包含在http请求中，改变#号后的路径不会刷新页面，对后端无影响。**即使没做到路由覆盖，也不会返回404错误**

history：地址栏没有#号，比较美观，可用history.state获取完整的路由信息。改变URL路径后会重新请求资源，**history模式下，前端URL向后端发起的URL一致，否则返回404错误**
